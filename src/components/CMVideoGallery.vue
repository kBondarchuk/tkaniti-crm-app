    ...
    <div :key="updateKey" :class="{ loading: isLoading }" class="gallery">
      <div v-for="image in records" :key="image.id" class="item" style="position: relative">
        ...
        <img
          ...
          alt="Удалить фото"
          title="Удалить фото"
          @click="deleteVideo(image.filename)"
        />
      </div>
...
export default {
  ...
  watch: {
    albumUuid: {
      ...
      handler(newValue) {
        // newValue, oldValue
        console.log("fetching videos for: " + this.albumUuid + ", newValue: ", newValue);
  ...
  methods: {
    ...
    deleteVideo(filename) {
      ...
      var confirmed = confirm("Вы действительно хотите удалить видео?");
      if (confirmed) {
        this.fetchVideoDelete(filename);
      }
    },
    // Network
    async fetchVideos(album_uuid) {
      console.log("1 fetchVideos for: ", album_uuid);

      this.isLoading = true;
      try {
        let result = await apiService.getVideos(album_uuid);
    ...
    async fetchVideoDelete(filename) {
      this.isLoading = true;
      try {
