<template>
  ...
  >
    <form class="ui error form" :name="$options.name" :class="{ loading: isLoading }" @submit.prevent>
      ...
      <UITextfield v-model="params.payload.return_url" label="Success callback URL" />
...
import PaymentsSettingsYooObject from "@/objects/PaymentsSettingsYooObject";

export default {
  name: "ModalPrefsYooItemEdit",
  ...
  props: {
    ...
    selectedItem: {
      type: Object,
      default: () => {
        return PaymentsSettingsYooObject;
      },
    },
  },
  ...
  data() {
    return {
      params: Object.assign({}, PaymentsSettingsYooObject),
      title: "Новые настройки Ю-кассы",
      isLoading: false,
      api_error: "",
    };
  },
  ...
  computed: {
    currentItem() {
      return this.selectedItem || PaymentsSettingsYooObject;
    },
  },
