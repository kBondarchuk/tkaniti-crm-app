<template>
  ...
  <div>
    <!-- Filter Form -->
    <CMReportForm :show-investors="false" @report-request="requestReport" @report-save="saveReport" />
    ...
    <table v-if="items.length" class="ui very compact small collapsing celled table" :class="{ loading: isLoading }">
      ...
      <tbody>
        <tr v-for="(item, index) in items" :key="index" :item="item">
          <td>{{ $filters.date(item.datetime, 0) }}</td>
          <td class="right aligned nobr">
            {{ $filters.money(item.amount) }}
          </td>
          <td>
            <a :href="'/contracts/lease/' + item.contract_id" @click.stop>{{ item.contract }}</a>
          </td>
          <td>{{ item.user }}</td>
          <td>{{ item.notes }}</td>
        </tr>
...
export default {
  ...
  methods: {
    requestReport(payload) {
      ...
      this.fetchReport("manual_deposits", payload.date1, payload.date2, payload.investors, payload.branches);
    },
    saveReport() {
